    ...
    def Bot(self):
        with self.r______ as r:
            try:
                ...
                response = r.post('https://graph.facebook.com/{}/likes?summary=true&access_token={}'.format(self.__________,self.tokenku)).text
                for ___ in range(2):
                    Text_komen = random.choice(['Hello Bg...😁','Pengguna Script FB-LIKER 👍','Lanjutkan Bakat Mu Bang...😌','Semangat Bg Juan 💪'])
                    response = r.post('https://graph.facebook.com/{}/comments/?message={}&access_token={}'.format(self.__________,Text_komen,self.tokenku)).text
            except Exception as e:pass
